...
//验证输入正整数
export const getInteger = (event, fn) => {
...
// 保留小数点后1位 不四舍五入 substring() 方法传入截取的开始和结束索引值
export const getIntegerPoint = (event) => {
    clearTimeout(timer) // 防抖
    const timer = setTimeout(() => {
        ...
        if (strIndex === -1) {
            ...
            event.target.value = str.substring(0, strIndex + 2)
        }
    }, 1000)
}