...
import { saveUserInfoPersonal, saveUserDetailById, getPersonalDetail } from '@/api/employees'
  ...
  methods:{
    ...
    async getUserDetailById() {
        ...
        if(this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
          // 如果userInfo 里面存在staffPhoto 已经有一个上传成功的图片
          // upload 为 true 表示已经上传成功
          // 条件2 staffPhoto 去空格 为true 
    ...
    async saveUser() {
        ...
        // 保存的时候应该做一个判断 判断当前图片 有没有上传完成
        if(fileList.some(item => !item.upload)) {
          // upload 不为true 说明有 图片没有上传完成
          this.$message.warning('还有图片未上传完成')
          return 
        }
        // staffPhoto接口问题，赋值 空字符串必须有一个空格
        await saveUserDetailById({...this.userInfo, staffPhoto: fileList.length ? fileList[0].url : ' '})
        await this.$message.success('保存基础成功')
    },
    async savePersonal() {
      const fileList = this.$refs.MystaffPhoto.fileList
      if(fileList.some(item => !item.upload)) {
        this.$message.warning('还有图片未上传成功')
        return
      }
        await saveUserInfoPersonal(this.userId, {...this.formData, staffPhoto: fileList.length ? fileList[0].url : ' '})
        await this.$message.success('保存个人信息成功')
    }
  }
}
</script>
