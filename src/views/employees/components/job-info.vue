...
import { saveJobInfo, getJobInfo, getEmployeeSiple } from '@/api/employees'
...
export default {
  ...
  methods: {
    async getJobInfo() {
      this.formData = await getJobInfo(this.userId)
    },
    // 保存岗位信息
    async saveJob() {
        await saveJobInfo(this.userId, this.formData)
        await this.$message.success('保存岗位信息成功')
    },
    // 获取员工列表 为 下拉列表提供数据
    async getEmployeeSiple() {
      this.depts = await getEmployeeSiple()
    }
  }
}
</script>
